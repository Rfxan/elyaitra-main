...
def get_provider() -> LLMProvider:
    ...
    if provider_type == "groq":
        try:
            ...
            print(f"⚠️ Groq provider setup failed ({e}). Falling back to Gemini.")
...
def get_embedding_provider() -> LLMProvider:
    """
    Dedicated provider for embeddings. 
    Groq doesn't support embeddings, so we fallback to Ollama or Gemini.
    """
    provider_type = config.LLM_PROVIDER
    
    # If using Groq for LLM, we MUST use something else for embeddings
    if provider_type == "groq":
        # Prefer Ollama for local embeddings if available, else Gemini
        try:
            return OllamaProvider(
                base_url=config.OLLAMA_BASE_URL,
                model_name=config.OLLAMA_DEFAULT_MODEL,
    ...
    # Otherwise, the primary provider (Gemini/Ollama) likely supports embeddings
    return get_provider()
