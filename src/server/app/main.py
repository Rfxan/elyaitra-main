...
app = FastAPI(title="Elyaitra Backend (Minimal)", version="0.1.0")
...
async def security_logging_middleware(request: Request, call_next):
    ...
    # EXCLUDE POLLS: Don't log the monitoring endpoint itself to avoid noise and buffer overflows
    # CRITICAL: Also exclude the security/log endpoint to prevent infinite recursion
    if path in ["/api/threats/logs", "/api/security/log", "/health", "/favicon.ico"]:
